...
import { IArticle } from '../models/articleModel.js';
        ...
        try {
            ...
            if (sortParam) {
                sortParam.split(',').forEach(part => {
                    const [field, order] = part.includes(':') ? part.split(':') : [part, 'asc'];
                    sort[field] = (order === 'desc') ? 'desc' : 'asc';
                });
            }

            const result = await this.service.getAllFiltered({ sort, page, limit });
            res.json(result);
        } catch (e) {
            next(e);
        }
